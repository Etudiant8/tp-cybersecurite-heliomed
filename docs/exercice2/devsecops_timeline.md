...
| **Build** (CI, à chaque commit) | #15 Scan SCA image (Trivy) · scan de secrets (gitleaks) · #14 Analyse statique OWASP sur le code applicatif | **Go/no-go CI — techniquement appliqué, pas seulement documenté** : `master` est protégé (GitHub branch protection, `enforce_admins: true`), les 2 checks qui scannent notre propre code (gitleaks, Trivy filesystem/IaC) sont **obligatoires avant tout merge**. Testé en conditions réelles : un push direct sur `master` est rejeté par GitHub (`GH006: Protected branch update failed`). Les 3 scans sur les images de démo (Juice Shop/DVWA) restent en mode rapport — elles sont volontairement vulnérables, les rendre bloquantes casserait le pipeline en permanence sans rapport avec notre propre code |
