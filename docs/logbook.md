...
| 2026-09-04 10:25 | Connexion VBoxManage guestcontrol (root/[REDACTED-ROTATED-PASSWORD], identifiants hérités de UBU-BASE) | Échec initial : `guestcontrol run` duplique le 1er argument après `--` en plus de --exe comme argv[0] → hostname changé par erreur en littéral "hostname". Corrigé en ne répétant plus le nom du programme après `--` | Claude Code : diagnostic de la syntaxe VBoxManage |
